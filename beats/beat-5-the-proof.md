...
You need someone with no memory of today to try it. That is what a **subagent** is: a separate session with its own context window, which knows nothing about your session.

### 1. Declare the tester

Subagents in this CLI are files, the same way Skills and rules are files. Create `.agents/agents/cold-tester.md`:

```markdown
---
name: cold-tester
description: Runs a Skill from scratch with no prior context and reports what it had to guess.
subagent: true
---

You have no memory of any earlier conversation. Follow the Skill you
are pointed at exactly as written. When you finish, list every point
where the instructions were unclear, what you had to decide for
yourself, and what you wish the Skill had said.
```
...
You should see `cold-tester` in the list.

### 2. Send it in cold

There is a second batch of tickets in the folder you have not touched. Point the subagent at it:

```text
Use the cold-tester subagent to run the ticket-themes Skill from
scratch on tickets-q2/. Give it no context beyond the Skill itself.
When it comes back, tell me what it found unclear, what it had to
guess, and what it wished the Skill had said.
```

That last sentence is the whole beat. You are not asking whether it produced a report. You are asking **what it had to guess**.

### 3. Read the complaint, not the output
...
- **It did not know which report to use as the baseline** — last quarter's, or the one you wrote this morning. Both are sitting in the folder now.
...
The baseline one is worth pausing on. It did not exist this morning. You created it by doing the work — your own output is now ambiguous input. That happens to real processes constantly.

### 4. Patch and run again
...
### What a good cold run looks like

In the dry run of this material, a clean-context subagent did **not** inherit Q1's story. It reported payment failures at 33 tickets (28%) and refunds not received at 29 (24%) — together 52% of the batch — tied them to the v4.5 payment-provider migration on 14 April, and noted that address-pin complaints had fallen to 6%.

It also came back with exactly two pieces of friction, and both are the ones you want the room to hit: which report to use as the baseline now that two exist, and how to break ties on a compound complaint where a payment failure causes a refund delay. Those are the missing sentences.

### The subagent-as-a-file thing needs one sentence, not five

Some attendees will not have used a subagent before. Introduce it once:

> *"A clean pair of eyes with no memory of this morning. In this CLI it is a file, like everything else."*

Do not lecture on context windows. Nobody needs the architecture to get the point.

**Have them declare the subagent in `.agents/agents/` rather than conjuring one mid-conversation.** Ad-hoc delegation with unusual tool bindings can fail with a `no tool converter registered` error, which is an unhelpful thing to debug with the clock running. A declared subagent inheriting the standard toolset just works. The file is on the participant tab; have it ready to paste.
